...
{
    const auto input_length = hex_str.size();
    const auto output_length = (input_length + 1) / 2;
...
Base64Decode(const char *encoded)
{
    ...
    const auto len = BIO_read(b64.get(), decoded.data(), int(maxlen));
    decoded.resize(len);
    return decoded;
}
...
PadCharVector(std::vector<unsigned char> &digest)
{
    ...
    auto hex_string = BinaryToHEXString(digest.data(), int(digest.size() * 2));
...
AWSUserAuthentication::AWSUserAuthentication(const std::string &username, const std::string &password,
...
void AWSUserAuthentication::getPasswordAuthenticationKey(const Aws::String &salt,
                                                         ...
                                                         std::vector<unsigned char> &key)
{
    ...
    auto srp_A_string_pad = PadHexStringWithLeadingZero(srp_A_string_);
    auto srp_b_string_pad = PadHexStringWithLeadingZero(srp_b); // can probably just add the hex strings... and convert to char
    auto srp_A_srp_b = srp_A_string_pad + srp_b_string_pad;
    ...
    const auto userid_string = user_pool_ + username_ + ":" + password_;
    ...
    const auto hex_s = std::string(BN_bn2hex(bn_S));
...
bool AWSUserAuthentication::InitiateAuthentication()
{
    ...
    Aws::InitAPI(options);
    {
        ...
        if (auth_request_result.IsSuccess())
        {
            ...
            const auto salt = challenge_map["SALT"];
            const auto srp_b = challenge_map["SRP_B"];
            const auto secret_block = challenge_map["SECRET_BLOCK"];

            auto claim64 = GenerateChallengeParameters(salt,
                                                              srp_b,
                                                              secret_block,
                                                              time_str);
            ...
            else
            {
                ...
                std::cout << "Request error: " << challenge_request_result.GetError().GetExceptionName() << " " << challenge_request_result.GetError().GetMessage() << std::endl;
                return false;
            }
        }
        else
        {
            ...
            std::cout << "Request error: " << auth_request_result.GetError().GetExceptionName() << " " << auth_request_result.GetError().GetMessage() << std::endl;
            return false;
        }
    }
